...
- **PixiJS visual production track:**
  ...
  - user-requested next graphics work is a production-art pass, not more symbols: improve real sprites, textures, animation states, damage/construction effects, and human-readable board motion.
  - next map-scale work should explicitly trial larger worlds in tiers, starting with a roughly 3x-current-area target around 384x384 tiles versus the current 224x224 world, then only keeping that scale if terrain chunking, pathfinding, resource distribution, zoom readability, and live-AI smoke runs remain stable.
...
1. **Development-tree balancing and unique mechanics:** keep the 191-node tree functional while deepening individual node mechanics, branch-specific consequences, long-run balance, and observer search UX. The first metadata contract is delivered; remaining work is to make more nodes meaningfully different in simulation over centuries. The rule remains: expose mechanics and consequences, not scripted strategy or build order.
2. **Chosen placement:** first AI-authored placement intent is delivered for wall/gate/turret/watchtower builds, straight multi-tile wall/gate perimeters execute through explicit LLM fields, and browser-hook overlay telemetry now groups authored fortification networks. Remaining work is richer perimeter shapes, route/blocking previews, and optional human confirmation only after the human play model is decided.
3. **Advanced siege behavior:** AI-authored siege/repair plans, executable retreat thresholds, artillery/projectiles, coordinated rally assaults, timed feint/probe withdrawal, staggered wave release, and repair-under-fire interruption are now active on real attack/repair tasks. Remaining work is multi-target groups, cover/escort assignments, richer contested repair counterplay, and more projectile/artillery variants.
4. **Gate diplomacy and strategy:** first flexible `GATE_OPERATION` order is delivered with freeform terms, tolls, expiry, lockouts through physical gate primitives, messenger delay/refusal/detention/ambush outcomes, operated-gate telemetry, first visual operated/safe-passage overlays, named treaty route evidence on real courier packets, factual treaty incident records when an active writ is restricted, and participant/witness scope for incident knowledge. Remaining work is richer witness ledgers with time-series observer memory, saboteur discovery, multi-gate route terms, and more detailed visual states for active writs, revoked writs, detained couriers, tolls, sabotage, and incidents.
5. **Fortification and item stat contract:** any future board item, inventory object, projectile, dropped packet, trade good, siege installation, or resource-processing installation must declare health, armor, attack, range, cooldown, browser-hook coverage, and destruction behavior when it can be targeted or can affect combat.
...
- Explicit wall/gate perimeters now execute through `perimeterPattern`, `perimeterDirection`, `perimeterLength`, and 1-based `perimeterGateIndex`; browser smoke proves a five-segment gate line with wall blocking, gate passability, visible sprites, and five plan records.
