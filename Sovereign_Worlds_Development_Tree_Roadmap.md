...
1. **Development-tree balancing and unique mechanics:** keep the 191-node tree functional while deepening individual node tradeoffs, synergies, social costs, aliases, UI search/filtering, and long-game balance. The rule remains: expose mechanics and consequences, not scripted strategy or build order.
...
3. **Advanced siege behavior:** expand beyond the first siege engine and breach-estimate slice into multiple siege tools, projectile/artillery entities, contested field repairs under fire, coordinated assaults, feints, retreat logic, and AI-authored assault plans.
...
6. **PixiJS graphics upgrade:** continue replacing the conceptual debug-board style with production-oriented PixiJS visuals. The first readable siege-engine texture is in; remaining work includes terrain/resource sprites, richer building silhouettes, wall/gate state art, animated units, selection/route previews, construction/damage/firing effects, and zoom-aware overlays.
7. **Survival scoring expansion:** expand beyond the first controlled/defended/raided/denied deposit modifier into defended logistics routes, happiness, safety, tribute, embargoes, market manipulation, and richer resource-denial consequences.
