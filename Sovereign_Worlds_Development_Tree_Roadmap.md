...
- **Scarcity design:**
  ...
  - normal browser starts should use a fresh map seed so humans and sovereigns do not see the same scarcity layout every run; explicit seed overrides remain available for reproducible QA and debugging.
  - resource deposits must be finite world objects: harvesting reduces the remaining amount, depleted deposits disappear or visibly become exhausted, and prompts/UI/browser hooks must show remaining value well enough for sovereigns to fight, trade, migrate, ration, or deceive around scarcity.
...
- **Survival mandate and learning tree hooks:**
  ...
  - One game-year should take roughly 20 real seconds at 1x speed. Every 100 game-years, the surviving population with the lowest total wealth dies out; the last survivor around year 400 wins.
  ...
  - Sovereigns must begin with no global roster or outside-nature knowledge: they do not know that any other sovereigns exist until evidence reaches them, they do not know how many outside powers exist, they do not know who those powers are, they do not know whether encountered or unseen actors are artificial, human, real-world, fictional, or any other out-of-world category, and they do not know which identities have been chosen elsewhere. Counterparties become known only through direct sight, delivered/sent messages, participant diplomacy, participant war, witnessed incidents, rumors, claims, or explicit information they independently obtain inside the world.
  - Sovereigns should be able to negotiate voluntary civilization mergers when both sides want it. A merger combines populations, territory, resources, claims, memories, diplomacy, and survival fate into one civilization, but it must have exactly one recognized leader/sovereign after the agreement. The leadership question should be negotiated explicitly through messages, concessions, threats, dynastic claims, abdication, councils, succession terms, betrayal risk, or other in-world terms the sovereigns invent.
...
- **PixiJS visual production track:**
  ...
  - the production art path should use real PixiJS `Sprite`/`AnimatedSprite` assets and spritesheet atlases for normal play: idle, walking, gathering, building, attacking, firing, damaged, repairing, opening/locking gate, courier travel, scouting, and death/ruin states.
  - remove reliance on map-letter/symbol placeholders for normal play; readable sprites, textures, silhouettes, and animation states are the production target, while abbreviations can remain only as optional debug/accessibility overlays.
  - improve the actual sprite and texture set until humans can read terrain, resources, units, fortifications, damage, and movement without relying on symbols; visual QA should treat unreadable placeholders as unfinished work.
  ...
  - the world-scale target is at least the current tripled-area baseline versus the original 128x128 map, with any further increase gated by smooth-frame QA, pathfinding cost, resource readability, and live-AI run stability.
  - later item 6 graphics work is a production-art pass, not more symbols: improve real sprites, textures, animation states, damage/construction effects, and human-readable board motion.
  - later item 6 map-scale work should explicitly trial larger worlds in tiers, starting with a roughly 3x-current-area target around 384x384 tiles versus the current 224x224 world, then only keeping that scale if terrain chunking, pathfinding, resource distribution, zoom readability, and live-AI smoke runs remain stable.
...
**Delivered combat-stat coverage and siege QA hardening on July 5, 2026:** every declared unit type, building type, and resource deposit type now has a type-level health, armor, attack/range, and cooldown stat helper covered by tests, including currently unspawned types such as traders. Browser building smoke now proves explicit siege orders destroy wall, gate, and turret targets through normal armor-reduced combat, and confirms wall/gate tiles become walkable once destroyed.

**Delivered resource-control posture slice on July 5, 2026:** resource deposits now expose controlled, contested, foreign-controlled, defended, hostile-defended, raided, under-attack, and denied posture. Survival safety scoring includes a bounded resource-control modifier, visible LLM prompts include known deposit posture without revealing unscouted rival deposits or exact rival wealth, resource denial events are only visible to observing/participating tribes, and reply scheduling no longer starves messenger answers under frame-pressure throttling.
...
**Delivered first development/siege/graphics capability slice on July 6, 2026:** the implemented development catalog expanded from the original core fortification nodes into a 16-node capability surface spanning masonry, brick kilns, ironworking, public works, ballistics, military architecture, siege engineering, road engineering, gate machinery, taxation, council governance, free press, propaganda, forced labor, abolition, and espionage. These nodes now change costs, happiness, safety, unit/building stats, repair speed, taxation, turret performance, gates/walls, and recruitable unit options. Siege engines are now a real recruitable unit unlocked by Siege Engineering, use the shared health/armor/attack contract, target hostile fortifications through normal `ATTACK targetBuildingId` orders, expose breach estimates, and have a readable Pixi texture plus browser smoke coverage. This slice deliberately exposes tools, costs, requirements, and consequences to LLM sovereigns without prescribing a fixed strategy or build order.

**Delivered full development-tree foundation on July 6, 2026:** the roadmap tree is now represented in the simulator as 175 `SW-*` developments plus the 16 stable core compatibility IDs, for 191 selectable developments. Generated roadmap nodes carry category, phase, costs, prerequisites, unlocks, and numeric effect metadata, and those effects feed generic simulation surfaces such as yearly wealth, happiness, safety, resource control, construction/repair cost, unit stats, building durability, building armor, building attack, and building range. LLM prompts and browser hooks expose bounded available-development summaries instead of dumping the whole tree, and unavailable `DEVELOP` or locked `BUILD` requests no longer auto-convert into hidden prerequisite strategy.

**Delivered AI-authored fortification/gate/siege intent foundation on July 7, 2026:** sovereign orders now preserve LLM-authored `fortificationIntent`, `perimeterShape`, `perimeterStrategy`, `gateOperationIntent`, `gateTerms`, `gateTollGold`, `siegeIntent`, `assaultPlan`, `retreatCondition`, and `repairPlan` fields instead of reducing every decision to primitive order types. The simulator records bounded fortification, gate-operation, and siege-plan histories; active gate operations can expire; messengers can be delayed, tolled, refused, detained, or ambushed at operated gates; and browser snapshots/selected gate UI expose recent plans. This deliberately records and executes authored tools and consequences without hardcoding a doctrine for any sovereign.

**Delivered explicit perimeter-construction slice on July 7, 2026:** LLM `BUILD` orders can now execute straight multi-tile `line` and `gate_line` wall/gate perimeters using explicit `perimeterPattern`, `perimeterDirection`, `perimeterLength`, and 1-based `perimeterGateIndex` fields. The simulator builds all segments through normal costs, development requirements, site reservation, visibility, events, and fortification-plan records; browser smoke proves four walls plus one central gate render visibly, wall tiles block movement, and the gate remains passable. This exposes an executable fortification tool without prescribing why, when, or where a sovereign should use it.

**Delivered executable siege-retreat slice on July 7, 2026:** LLM `ATTACK` orders can now include `retreatHealthPct` plus optional `retreatX`/`retreatY` rally coordinates. Sovereigns still write their own `retreatCondition`, `siegeIntent`, and `assaultPlan`; the numeric threshold is the explicit executable safety trigger. Assigned siege attackers pull back into normal movement when damaged below the threshold, emit `SIEGE_RETREAT_TRIGGERED`, and retreating artillery is prevented from opportunistically firing at the abandoned wall.

**Delivered repair-under-fire interruption slice on July 7, 2026:** hostile combat pressure at a damaged structure now interrupts active `REPAIR` ticks instead of letting crews heal fortifications through direct threat. Repair tasks keep `lastInterruptedTick`, events emit `REPAIR_UNDER_FIRE_INTERRUPTED` with the threatening unit and LLM-authored intent, and browser smoke proves the wall stays damaged while pressure remains. This is a world rule and tool surface, not a prescribed strategy: sovereigns remain free to clear, cover, negotiate, deceive, retreat, sacrifice, or delay based on their own plans.

**Delivered strategy-boundary and gate QA hardening on July 7, 2026:** `SET_POLICY` prose no longer converts into executable `DEVELOP` or `BUILD` orders; sovereigns must use the explicit tools they intend to execute. `GATE_OPERATION` no longer treats generic `subject`/`body` as private terms or public notices, messenger packets bind gate outcomes to route-relevant recipient gates, and remote operated gates cannot control unrelated arrivals. Plain `line` perimeters are wall-only, while gate-bearing perimeters require explicit `gate_line`.

**Delivered coordinated-assault and feint/probe execution slice on July 7, 2026:** LLM `ATTACK` orders can now include `assaultMode` (`direct`, `coordinated`, `feint`, or `probe`), explicit `assemblyX`/`assemblyY`, optional `assaultDelayTicks`, and optional `feintDurationTicks`. Coordinated attackers rally in an `assembling` phase before release, emit `COORDINATED_ASSAULT_STARTED`, and then attack normally; timed feints/probes withdraw to the commanded retreat point and emit `SIEGE_FEINT_WITHDRAWAL`. The engine executes only the explicit fields and does not infer these strategies from prose.

**Delivered observer/world-scale/population foundation on July 7, 2026:** the board is now 224x224 tiles, roughly tripling playable area versus the original 128x128 map, with scaled starts and more spread-out contested deposits. Observer mode now shows ranked tribe rows with score, wealth, population/capacity, happiness, safety, military, diplomacy, houses, and fortification counts. Houses are a real buildable structure, raise population capacity by 8, appear in UI/build hooks, and safe, happy, fed populations can grow yearly when capacity exists. LLM prompts expose population capacity, house costs, and growth tradeoffs without prescribing whether a sovereign should expand.

**Delivered explicit siege-wave slice on July 7, 2026:** LLM `ATTACK` orders can now include `assaultWaveSize` and `assaultWaveIntervalTicks` to stagger assigned attackers into authored waves. Waiting wave units cannot damage the target before their release tick, release events are recorded as `SIEGE_WAVE_RELEASED`, and siege-plan catch-up exposes wave size, interval, and released waves. This is an executable timing tool, not a prescribed assault doctrine.

**Delivered fortification overlay telemetry slice on July 7, 2026:** `render_game_to_text().boardReadability.fortificationOverlay` now summarizes visible wall/gate/turret/watchtower counts, player-blocking/passable wall-gate segments, operated gates, active access treaties, active and historical sabotage, and LLM-authored perimeter groups derived from real `fortificationPlans`. Browser smoke now proves a five-segment `gate_line` perimeter appears as one authored group with four blocking walls and one passable gate, and proves operated-gate/sabotage history is visible through the overlay. This is observer/readability instrumentation only; it does not prescribe any sovereign strategy.

**Delivered visual fortification overlay slice on July 7, 2026:** the defense overlay now draws visible blocked-route markers, safe-passage gate corridors, operated-gate status, authored perimeter group centers/joins, and war-front contact markers. `render_game_to_text().boardReadability.fortificationOverlay.visualOverlayMarkers` mirrors those visual rules with marker counts and bounded ids/pairs. Browser smoke proves blocked wall/gate markers, operated-gate and safe-passage markers, a five-segment perimeter center/group marker, and a repair-under-fire Blue/Red war-front marker. This remains observer/readability instrumentation; it does not add strategic consequences or prescribe sovereign behavior.

**Delivered resource-pressure overlay slice on July 7, 2026:** `render_game_to_text().boardReadability.resourcePressureOverlay` now exposes route, contested, scarce, raided, and recently denied resource markers, with bounded pressure-site ids and marker counts. The Pixi overlay draws route lines toward strategic deposits and keeps a visible denied-resource marker even after a deposit has been destroyed. Browser smoke proves a forced iron raid at `iron@49,52` produces a recent denial marker. This is observer/readability instrumentation only; it does not prescribe any sovereign strategy or resource doctrine.

**Delivered named safe-passage treaty route slice on July 7, 2026:** explicit `GATE_OPERATION` access-treaty grants now leave route evidence on real messenger packets when an active named treaty makes an owner-only gate passable for a counterparty. Packet route memory names the treaty id/name and gate, `render_game_to_text().packets[].routeMemory` exposes the evidence, and sovereign prompts include recent courier route evidence in gate diplomacy state. Tests prove chat alone does not open gates, a named grant enables a real Red courier path through a Blue owner-only gate, revocation blocks a fresh courier again, and browser smoke sees the same route evidence. This creates factual diplomatic evidence without deciding whether any sovereign should trust, punish, exploit, or betray the treaty.

**Delivered treaty incident evidence slice on July 7, 2026:** when an active gate access treaty exists for a courier and the gate owner applies a restrictive outcome (`refuse`, `detain`, or `ambush`) through an explicit gate operation, the simulator now records a bounded `gateTreatyIncidents` ledger entry naming the treaty, gate, packet, operation, owner, affected tribe, action, and toll context. Packet route memory, `render_game_to_text().recentGateTreatyIncidents`, sovereign prompts, and browser smoke expose the incident as factual evidence only. The engine does not infer trust, betrayal, punishment, alliance changes, or war from the incident; each sovereign must decide its own response.

**Delivered treaty incident witness-scope slice on July 7, 2026:** treaty incidents now distinguish direct participants from third-party witnesses. The simulator records `participantTribeIds` and `witnessTribeIds` based on current direct vision around the gate at incident time, browser smoke proves a nearby Green sentinel witnesses a Blue/Red gate incident, and prompts split the wording by perspective: participants receive treaty incident context, while witnesses receive observed gate-action evidence with treaty details explicitly unverified. This adds information flow without leaking witness names through packet route prose and without adding automatic diplomatic consequences.

**Delivered hidden-sovereign discovery boundary on July 7, 2026:** sovereign prompts no longer expose the global tribe roster, total survivor count, startup tribe count, chosen identities elsewhere, recipient-AI wording, model names, machine/human/simulation labels, or routine off-map event broadcasts. Known counterparties are now derived from actual contact: sight, delivered or sent messages, direct diplomacy, alliance/war participation, gate treaty facts, witnessed incidents, and diplomatic intel. Routine self-actions are owner-visible, combat/gate evidence is participant/direct-vision scoped, and observer UI remains separate from prompt-visible sovereign knowledge.

**Delivered finite resource depletion and randomized-start slice on July 8, 2026:** normal harvesting now creates bounded `ResourceDepletionRecord` evidence when a finite deposit is exhausted, emits `RESOURCE_DEPOSIT_DEPLETED`, invalidates resource-control scoring, preserves partial final cargo for delivery, and exposes recent depletions through `render_game_to_text().recentResourceDepletions` plus `boardReadability.resourcePressureOverlay.depletionMarkers`. The Pixi resource-pressure overlay draws a distinct exhausted marker separate from attack-denial markers, selected depleted tiles report the exhausted resource, and browser smoke proves a real peon gather path exhausts a clay deposit without invisible continued production. Default simulator/browser starts now use fresh random seeds, explicit `?seed=...` and `createGame(seed)` preserve deterministic scarce-resource layouts for QA, and `resourceScarcityPolicy` centralizes coal/iron scarcity so UI overlays and tests follow the same world rule. Browser hooks now expose a compact `resourceLayoutFingerprint` covering live resource type, position, finite amount, and HP, and smoke QA fails if fresh starts reuse the same layout or if explicit seeded starts do not replay it.

**Delivered negotiated civilization merger slice on July 8, 2026:** sovereigns can now propose voluntary civilization mergers through authored `MERGER_PROPOSAL` messenger text with explicit `MERGER_OFFER`, `mergerLeaderTribeId`, and `mergerTerms`; a merger executes only when the reply uses `MERGER_ACCEPT` and repeats the same proposed sole leader. The simulator transfers the merged civilization's living units, buildings, resources, developments, diplomacy relationships, war state, population cap, and survival fate into the leader; the absorbed sovereign becomes a retired sentinel rather than a second ruler, and the merged tribe stops scheduling separate identity, reply, or strategy turns. `civilizationMergers`, tribe merged status, victory merged counts, terms, and memory evidence are exposed through `render_game_to_text()`, browser smoke exercises the real messenger/reply path, and leadership is never auto-decided by score or hardcoded preference.

**Delivered first development-tree metadata pass on July 8, 2026:** all 191 developments now expose first-class aliases, tradeoffs, synergies, and social-cost metadata in addition to costs, prerequisites, unlocks, and numeric effects. Generated `SW-*` nodes derive this metadata from category, name, cost pressure, and effect shape; core nodes get the same normalized contract. LLM order availability now includes compact metadata without crowding out executable attack/resource targets, and observer/browser telemetry exposes category/phase filters plus searchable text fields. This remains descriptive capability data, not a scripted strategy or build order.

**Delivered generated-development effect specialization slice on July 9, 2026:** generated `SW-*` roadmap nodes now infer more differentiated deterministic mechanics from institution semantics, including tax systems, trade/tariff corridors, finance, coercive labor, public information, controlled media/secrecy, law enforcement, fortifications, safe-conduct diplomacy, siege, ranged defense, and logistics. New target buckets such as `courier`, `watch`, `ranged`, `ranged_defense`, `siege`, `law_enforcement`, and `defensive_building` let effects apply to narrower unit/building families instead of only broad categories. Focused simulator tests prove same-category roadmap nodes now have distinct effect signatures and that trade corridors improve couriers without silently changing unrelated sentinels.

**Delivered fortification placement-preview slice on July 8, 2026:** every authored fortification plan now carries a factual `placementPreview` over the resolved tiles actually built, not just the requested target. The simulator records blocking segment ids, gate passability for the owner and nearest foreign town, route checks with and without the placed structures, route-blocked counts, and max added route steps. Perimeter groups inherit an aggregate preview, observer overlays draw route-impact lines from the simulator record, browser smoke verifies the preview for a five-segment wall/gate line, and sovereign catch-up prompts summarize the preview without recommending what strategy to choose.

**Delivered multi-target siege group slice on July 8, 2026:** `ATTACK` orders can now carry `targetBuildingIds` with up to five exact visible hostile structure ids, preserving `targetBuildingId` for single-target compatibility. The simulator validates that the group is hostile, alive, and same-target-civilization, records the authored group on `SiegePlanRecord`, splits assigned military units across the explicit targets, and retargets surviving units only within that same AI-authored group when their current target falls. LLM schema, prompt availability, catch-up summaries, sim tests, and browser smoke now cover the capability without inventing target priorities or deciding strategy for the sovereign.

**Delivered explicit siege cover/escort role slice on July 9, 2026:** `ATTACK` orders can now assign exact own militia/archer ids to `coverUnitIds` and `escortUnitIds`, with independent anchors, radii, and freeform plans. The simulator validates ownership, unit type, aliveness, and non-overlap, records the roles on `SiegePlanRecord`, removes those units from breaching assignments, and gives them guard tasks that intercept hostile units around their sovereign-authored positions. LLM schema, prompts, catch-up summaries, browser task text, and smoke coverage expose the tool as evidence and execution, not as a hardcoded tactic.

**Delivered contested repair interdiction slice on July 9, 2026:** `ATTACK` orders against exact hostile buildings can now set `interdictRepairs`, `repairInterdictionPlan`, and optional `repairInterdictionRadius`. When the sovereign chooses this tool, militia/archers assigned to the attack suppress hostile repair crews actively working on the same target before resuming structural pressure. Repair-under-fire now has a tested resume edge after pressure clears, siege plans preserve interdiction intent, and browser smoke proves the live hook damages a repairer and emits factual evidence. This gives AIs a counter-repair tool without deciding when they should use it.

**Delivered turret/watchtower line perimeter slice on July 9, 2026:** `BUILD` orders can now use `perimeterPattern: "line"` with `buildingType: "turret"` or `"watchtower"` to place authored multi-structure defensive or lookout lines through the same perimeter command surface used by walls. The simulator preserves segment/group records, charges real costs, respects development requirements, records placement previews, and rejects `gate_line` for tower perimeters so no hidden gate doctrine is inferred. LLM prompts expose the option as a tool the sovereign may choose; browser smoke proves turret and watchtower perimeter groups appear in observer telemetry and overlays.

**Delivered projectile visualization and stat-contract slice on July 9, 2026:** catapult stone-shot projectiles now have a projectile-type registry and type-level combat stats, live projectiles inherit that contract while preserving computed impact damage, `combatStatCoverage` checks projectile types as well as live projectiles, and browser hooks expose projectile `hp`, `maxHp`, `armor`, `attack`, `range`, and `attackCooldown`. The Pixi projectile draw path now renders a larger directional stone shot with glow and trail, and browser smoke captures a dedicated mid-flight projectile screenshot so the visual cannot silently disappear after impact.

**Delivered factual combat opportunity brief slice on July 9, 2026:** sovereign prompts now include a neutral `Combat opportunity brief` that gathers battle-relevant evidence in one place: known contacts, active wars, own military capacity, visible foreign units, visible structure targets with exact ids and breach estimates, and visible raid/denial resource coordinates ranked by conflict and scarcity. The brief explicitly says it is evidence only, so sovereigns remain free to fight, negotiate, scout, defend, deceive, ignore, or prepare rather than following hardcoded attack doctrine.

**Delivered gate treaty witness-memory slice on July 9, 2026:** treaty incidents now append bounded witness-only observations to the existing foreign-observation stream for each direct-vision witness. Witness observations identify the incident id, gate, owner, affected courier side, packet, action, and tick while keeping treaty details explicitly unverified. Sovereign catch-up prompts surface those observations as scouting evidence, browser telemetry exposes them through `gateTreatyWitnessObservations`, and repeated incidents accumulate as time-series evidence without automatic war, alliance, trust, punishment, or reputation changes.

**Delivered pre-build fortification preview slice on July 9, 2026:** `previewFortificationBuild()` now resolves wall/gate/turret/watchtower placement through the same site reservation, perimeter geometry, development checks, cost calculation, gate passability, and route-impact preview machinery used by real `BUILD` orders, but runs on synthetic preview buildings before resources are spent. Browser smoke proves the preview for a five-segment wall/gate line reports four blocking walls, one passable gate, route checks, affordability, and exact would-build counts while leaving resources, buildings, and fortification-plan records unchanged until the sovereign actually executes the build.

**Delivered ranged projectile visualization slice on July 9, 2026:** archer fire and turret fire now create short-lived visible combat projectile entities, `arrow` and `turret_bolt`, in addition to catapult `stone_shot`. These projectiles carry the same health, armor, attack, range, cooldown, target-kind, origin, and browser-hook metadata as artillery projectiles, while preserving existing immediate archer/turret damage timing. Pixi draws arrows as shafts with heads and turret bolts as bright fast bolts, and browser smoke captures them through the projectile screenshot without adding any AI targeting doctrine.

**Delivered observer combat discoverability slice on July 9, 2026:** projectile launches, projectile impacts, structure destruction, resource raid destruction, and messenger kills now carry optional structured event context with board coordinates, actor/target tribes, target ids, projectile type, and severity. `render_game_to_text()` exposes bounded `recentCombatEvents` plus `boardReadability.combatOverlay` with recent combat markers and active projectile markers, while the Pixi board draws lightweight pulse rings around recent visible combat and live projectiles. The existing event log now surfaces a compact recent-combat block for off-camera fights. This is observer/readability instrumentation only; it does not create wars, select targets, alter fallback behavior, or prescribe sovereign strategy.

**Delivered total-wealth, private-intelligence, and projectile-sprite QA slice on July 9, 2026:** century-review ranking, culling, public pressure text, prompts, and learning now use lowest **total wealth**, not wealth per capita or a per-capita-derived score. `REQUEST_INFO` is explicitly validated and prompted as private intelligence to a sovereign's own clerks/scouts; peer-directed questions such as asking another ruler to reveal gold reserves must use physical `SEND_MESSENGER` chat and written replies. Projectile rendering now uses pooled Pixi sprites for arrows, turret bolts, and stone shots, with browser telemetry proving active projectile sprite counts. The renderer caps Retina resolution, disables antialiasing for sharper/lighter map sprites, lowers concurrent Ollama lanes, and raises the active smoothness floor to 25 FPS while preserving the no-hardcoded-strategy boundary.

**Delivered corner perimeter placement slice on July 9, 2026:** `BUILD` orders can now use explicit `perimeterPattern: "corner"` and `perimeterPattern: "gate_corner"` to create L-shaped wall/gate/turret/watchtower perimeter groups from the same authored geometry fields used by straight lines. `gate_corner` uses the 1-based `perimeterGateIndex` to place one commanded gate segment at the bend or another explicit segment, while `perimeterShape` remains descriptive prose only. Simulator previews and real builds share the resolver, preserve segment/group records, charge normal costs, and record route/blocking placement previews; browser smoke proves a five-segment gated corner renders as one visible group with four blocking walls and one passable gate.

**Delivered saboteur discovery evidence slice on July 9, 2026:** foreign gate sabotage now records the nearby unit that executed the sabotage on active gate-sabotage state, sabotage history, and observer-facing scouting observations when the gate owner or another tribe has direct vision. `gate_sabotage_witnessed` observations identify the saboteur tribe/unit, gate, action, sabotage id, and operation id; sovereign catch-up prompts present this as factual sighting evidence only. Browser telemetry exposes `gateSabotageWitnessObservations`, and smoke proves the observer can see the actor identity without triggering automatic war, alliance changes, punishment, trust, or reputation logic.

**Delivered gate-diplomacy marker state slice on July 9, 2026:** the observer defense overlay now has explicit bounded marker telemetry and Pixi badges for active access writs, revoked writs, toll gates, detained-courier gates, treaty-incident gates, sabotage history, and witnessed discovery evidence. Browser smoke proves these marker families on a real gate using the existing grant/revoke/detain/sabotage fixture. This remains visual/readability instrumentation only: it does not decide trust, war, retaliation, access policy, or any sovereign strategy.

**Delivered frame-loop decoupling and active-AI cadence guard on July 9, 2026:** simulation ticks no longer launch LLM work directly inside the render callback. They queue a paced post-frame AI pump, start at most one LLM job per pump, and delay possible second-lane starts long enough for FPS metrics to reflect the first model's load. Concurrency now drops to one unless the browser is comfortably near 60 FPS. Browser smoke uses a ticker-owned motion probe and a mocked active Ollama lane to prove multiple rendered frames per simulation tick, same-tick visual motion, `maxTickerDeltaMs <= 160`, and `measuredFps >= 25`.

**Delivered multi-gate route terms slice on July 9, 2026:** explicit `GATE_OPERATION` access-treaty grants and revokes can now name a safe-passage route across multiple owned living gates using `gateRouteName`, `gateRouteGateIds`, and `gateRouteTerms`. The simulator creates matching per-gate treaty records under one shared route id, rejects foreign or destroyed route gates, preserves route terms in courier route memory, exposes route metadata in prompts, selected-gate UI, browser telemetry, and smoke tests, and keeps passability tied to the explicit listed gates only. This is a route tool and evidence surface, not a strategy script.

### Active Next Five Roadmap Items

The current implementation scope is **only** the following five numbered items. Later roadmap items remain important, but they are parked until these five are advanced or explicitly reprioritized.

**Active-five performance gate:** every implementation slice under items 1-5 must preserve smooth-frame playback at **at least 25 FPS**. QA must include `SOVEREIGNS_URL=http://127.0.0.1:5173/ pnpm smoke:smooth` or an equivalent browser smooth-frame check that proves an active mocked AI lane, `measuredFps >= 25`, ticker-owned same-tick visual motion across multiple rendered frames per simulation tick, `maxTickerDeltaMs <= 160`, no browser errors, and inspected screenshot evidence; a slice is not closed if it drops below that floor or collapses movement back to one visible frame per turn.

**Story closure rule:** once a roadmap story is built, it must be QAed, documented in `progress.md` with the exact evidence, and explicitly closed before moving on. If QA is incomplete, the story stays open and the next item does not start except for read-only planning.

1. **Development-tree balancing and unique mechanics:** keep the 191-node tree functional while deepening individual node mechanics, branch-specific consequences, long-run balance, and observer search UX. The first metadata contract and first generated-effect specialization pass are delivered; remaining work is broader long-century balance, additional branch-specific counter-effects, and stronger observer comparison/search UX. The rule remains: expose mechanics and consequences, not scripted strategy or build order.
2. **Chosen placement:** first AI-authored placement intent is delivered for wall/gate/turret/watchtower builds, straight multi-tile wall/gate/turret/watchtower line perimeters execute through explicit LLM fields, browser-hook overlay telemetry groups authored fortification networks, route/blocking placement previews are exposed from simulator pathing, and pre-build preview now resolves exact wall/gate line plus L-shaped `corner`/`gate_corner` placement before spending resources or creating buildings. Remaining work is richer perimeter shapes beyond straight lines and L corners, broader pre-build preview surfaces for additional authored shapes, and optional human confirmation only after the human play model is decided.
3. **Advanced siege behavior:** AI-authored siege/repair plans, executable retreat thresholds, artillery/projectiles, pooled visible mid-flight projectile sprites for `stone_shot`, `arrow`, and `turret_bolt`, observer combat-discoverability markers, factual combat opportunity briefs, coordinated rally assaults, timed feint/probe withdrawal, staggered wave release, repair-under-fire interruption/resume, explicit multi-target siege groups, explicit cover/escort guard assignments, and explicit repair-interdiction attacks are now active on real attack/repair tasks. Remaining work is more nuanced per-unit role behaviors beyond current cover/escort/interdiction tools, longer live-AI war/raid soak evidence, and more projectile/artillery variants beyond the current stone shot, arrow, and turret bolt.
4. **Gate diplomacy and strategy:** first flexible `GATE_OPERATION` order is delivered with freeform terms, tolls, expiry, lockouts through physical gate primitives, messenger delay/refusal/detention/ambush outcomes, operated-gate telemetry, visual operated/safe-passage overlays, named treaty route evidence on real courier packets, factual treaty incident records when an active writ is restricted, participant/witness scope for incident knowledge, time-series witness observations for repeated treaty incidents, factual saboteur-discovery observations for visible foreign gate sabotage, explicit observer marker states for active/revoked writs, tolls, detained couriers, incidents, sabotage, and discoveries, and explicit multi-gate route terms across exact owned gates with shared route ids. Remaining work is longer live-AI negotiation evidence for whether sovereigns invent, bargain over, exploit, and honor complex gate arrangements without scripted strategy.
5. **Fortification and item stat contract:** any future board item, inventory object, projectile, dropped packet, trade good, siege installation, or resource-processing installation must declare health, armor, attack, range, cooldown, browser-hook coverage, and destruction behavior when it can be targeted or can affect combat. Projectile type-level coverage is now delivered for `stone_shot`, `arrow`, and `turret_bolt`; active work is to keep enforcing this contract in schema, simulator validation, browser hooks, and tests for each new targetable or combat-effective item.

### Later Roadmap Items

These items are outside the current active-five scope unless the user explicitly changes priority.

6. **PixiJS graphics upgrade:** continue replacing the conceptual debug-board style with production-oriented PixiJS visuals. The first readable siege-engine and house texture slices are in, and the board is now scaled for a larger world. Remaining work includes significantly improved terrain/resource sprites, richer unit and building sprites, wall/gate/turret state art, sprite-atlas/AnimatedSprite animation states for walking/gathering/building/combat/gate/turret/courier/scout/death states, selection/route previews, construction/damage/firing effects, and zoom-aware overlays. Normal play should read through sprites/textures/animation, not symbols. Preserve or improve the current tripled-area map baseline; next world-scale trials should test roughly 320x320 and 384x384 maps, with 384x384 representing about triple the current 224x224 playable area, and keep the larger size only if smooth-frame, pathfinding, resource-conflict, and live-AI QA pass.
7. **Survival scoring expansion:** first controlled/defended/raided/denied deposit scoring and first visible resource-pressure route/denial overlays are delivered. Remaining work is defended logistics routes, happiness/safety interactions, tribute, embargoes, market manipulation, and richer resource-denial consequences that affect the world without hardcoding sovereign strategy.
8. **Map readability overlays:** first fortification-network telemetry/visual overlays and first resource-pressure route/denial overlays are delivered. Remaining work is richer contested-resource route previews, zoom-aware label tiers, stronger large-map clutter control, and trend/observer overlays that stay readable during long AI runs.
...
10. **Post-game learning QA:** every win-condition slice must prove century reviews eliminate the surviving population with the lowest total wealth, the year-400 survivor wins, and one individualized learning note is written per sovereign for the next iteration.
11. **Population growth balance and AI uptake:** houses and settlement developments already create capacity, and yearly growth already depends on food, happiness, safety, and capacity. Remaining work is long-run QA that sovereigns recognize low population as a safety/labor risk, choose housing/food/growth differently across models, and are not all suppressed by identical fallback behavior or over-tight growth thresholds.
12. **Hidden-world knowledge QA:** keep prompts encounter-scoped. Regression tests must prove fresh sovereigns do not receive other tribe ids/names, total tribe/survivor counts, unseen identity lists, model/provider names, machine/human/simulation labels, or claims that other sovereigns definitely exist before evidence. Delivered contact, direct sight, alliances, wars, rumors, claims, and witnessed incidents may reveal only the specific counterparties or alleged counterparties involved, without revealing a global roster or count.
13. **Voluntary civilization merger:** explicit negotiated merger execution is delivered through real messenger proposals and replies. Sovereigns can propose, debate, accept, reject, lie about, or counter a plan to merge civilizations; execution requires `MERGER_ACCEPT` with the same proposed sole leader. The resulting civilization has one leader, one population, one resource pool, one diplomacy state, and one survival fate, and observer telemetry preserves the negotiated terms. Remaining work is richer long-form merger negotiation memory, succession/council/internal legitimacy consequences, UI filtering for absorbed tribes in observer rankings, and long-run live-AI QA that sovereigns discover and use mergers without scripted strategy.
14. **Finite resource depletion:** normal harvesting now reduces finite deposits, removes exhausted deposits, records recent depletion evidence, exposes exhausted markers in browser hooks/UI, and proves no invisible continued production through sim and browser smoke. Remaining work is long-run balance around depletion rates, richer exhausted terrain/sprite states, sovereign prompt summaries for depletion trends, and strategic consequences for exhausted local economies without hardcoding responses.
...
## Current Implementation Backlog Status - 2026-07-07

### Completed In Active Siege Slice

- `battering_ram` and `catapult` are now implemented as real trainable unit tools, not hardcoded strategies.
- Rams and catapults are gated by development prerequisites, costs, and production capacity.
- Rams and catapults expose health, armor, attack, range, and cooldown stats through the same combat-stat QA contract as all other board objects.
- Catapults create visible `SiegeProjectile` records with stat coverage, launch/impact events, deterministic world signatures, and Pixi board drawing.
- Browser smoke now proves the full artillery path: unlock, train, issue a real `ATTACK`, see an in-flight projectile, impact the wall, and destroy it.
- LLM prompts/schema expose the new tools while preserving sovereign-authored `siegeIntent`, `assaultPlan`, and `retreatCondition` without prescribing a doctrine.
- Gate diplomacy now uses explicit mechanical fields instead of keyword parsing: `gateEntryAction`, `gateTollMode`, `gateUnpaidAction`, `gateTollGold`, and `gatePublicNotice`.
- Private/deceptive gate terms are no longer broadcast globally; owner-only private gate orders are split from deliberate public gate notices.
- Browser smoke now proves explicit gate action, toll mode, unpaid action, public notice, and private terms through the real `GATE_OPERATION` order path.
- Detained messenger packets can now be explicitly held, released, or ransomed through `gateDetainedPacketAction`, `gateDetainedPacketId`, `gateRansomGold`, `gateReleaseSubject`, and `gateReleaseMessage`; ransom uses real gold transfer and release returns the original courier through normal pathfinding.
- Controlled passage can now be granted or revoked through explicit gate access treaty records, and gate sabotage can force gates open, jam them shut, damage them, or clear sabotage through explicit commands; foreign gate sabotage requires a nearby living non-messenger unit.
- Explicit wall/gate perimeters now execute through `perimeterPattern`, `perimeterDirection`, `perimeterLength`, and 1-based `perimeterGateIndex`; browser smoke proves five-segment `gate_line` and `gate_corner` groups with wall blocking, gate passability, visible sprites, and five plan records.
- Executable siege retreats now use `retreatHealthPct` plus optional `retreatX`/`retreatY`; browser smoke proves damaged attackers abandon the wall, move to the rally point, and retreating artillery does not fire at the abandoned wall.
- Repair-under-fire now interrupts repair ticks while hostile combat units threaten the crew or damaged structure; browser smoke proves the wall remains damaged, interruption events fire, and the selected panel keeps repair state readable.
- AI strategy boundaries are hardened: policy prose is no longer converted into executable build/development strategy, explicit gate terms/public notices are required, remote gates no longer control unrelated messenger arrivals, and plain line perimeters cannot silently insert a gate.
- Coordinated assaults and feints/probes now execute through explicit fields: `assaultMode`, `assemblyX`/`assemblyY`, `assaultDelayTicks`, `feintDurationTicks`, and `retreatX`/`retreatY`; browser smoke proves assembly release and timed withdrawal.

### Remaining Near-Term Backlog

- Observer UI now has a live ranked tribe panel with core stats; remaining work is trend deltas, richer resource breakdowns, and better long-run comparison charts.
- World presentation now has the larger map foundation; remaining work is production sprite/texture atlases, clearer terrain/resource/building art, real PixiJS sprite/AnimatedSprite states, and large-scale readability for scouting, borders, roads, walls, armies, and resource conflicts. The next map-scale trial should evaluate roughly 320x320 and 384x384 worlds before committing to another scale increase.
- Population growth now has houses, capacity, yearly growth, and AI-visible tradeoffs; remaining work is long-run balance, more settlement/development synergies, and QA that AIs choose growth differently across models rather than all converging or staying artificially small.
- Authored fortification intent now has first wall/gate network overlay readability; remaining work is richer perimeter shapes, blocked-route previews, stronger operated-gate overlays, and better visual joins.
- Siege behavior now has explicit staggered wave release; remaining work is multi-target wave groups, cover/escort assignments, richer repair-under-fire counterplay, more artillery/projectile variants, and better Pixi animation polish.
- Gate diplomacy still needs better neutral diplomatic evidence and tooling: treaty-breach detection, witness/event records, saboteur discovery, operated-gate overlays, and AI-readable context so each sovereign can decide its own response rather than the simulation hardcoding punishment or strategy.
- Development-tree balance still needs tuning across the 191 implemented nodes so civilizations diverge through meaningful tradeoffs rather than only catalog breadth.
